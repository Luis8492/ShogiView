...
- 「Go to start / Step back / Step forward / Go to end」ボタンで手順を順送り・巻き戻し
- ←/→・Home/End キーで局面を移動できるキーボードショートカットを提供
- スペースキーまたはツールバーの「Start autoplay / Pause autoplay」ボタンで自動再生を開始・停止
- 任意の手数を指定してその局面から再生を開始可能
- 分岐棋譜に対応し、現在の分岐を示すパンくず表示・「Return to parent line」ボタン・分岐選択ドロップダウンを提供
